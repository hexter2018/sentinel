...
def is_valid_parking_address(address, network='mainnet'):
    ...
    parking_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Parking address
        return False

    if (address_version != parking_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.parkingd_serialise())
...
def SHIM_serialise_for_parkingd(sentinel_hex):
    ...
    # superblock => "trigger" in parkingd
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # parkingd expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    parkingd_hex = serialise(obj)
    return parkingd_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('parking.conf')
