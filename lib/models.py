...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, parkingd):
        golist = parkingd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_parkingd(parkingd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_parkingd(self, parkingd, rec):
        import decimal
        import parkinglib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from parkingd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from parkingd, with every run
        ...
        # get/create, then sync payment amounts, etc. from parkingd - Dashd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(parkingd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from parkingd! %s" % e)
            govobj.vote_delete(parkingd)
            return (govobj, None)
    ...
    def vote_delete(self, parkingd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(parkingd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, parkingd, signal, outcome):
        import parkinglib
        ...
        output = parkingd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = parkinglib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(parkingd, signal)

    def sync_network_vote(self, parkingd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = parkingd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = PARKINGD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 parking addr, non-multisig
            if not parkinglib.is_valid_parking_address(self.payment_address, config.network):
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (parkinglib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = PARKINGD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not parkinglib.is_valid_parking_address(addr, config.network):
    ...
    def hash(self):
        import parkinglib
        return parkinglib.hashit(self.serialise())
