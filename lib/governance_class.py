...
class GovernanceClass(object):
    ...
    def vote(self, parkingd, signal, outcome):
        return self.go.vote(parkingd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, parkingd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(parkingd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(parkingd, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import parkinglib
        obj_data = parkinglib.SHIM_serialise_for_parkingd(self.serialise())
    ...
    def parkingd_serialise(self):
        import parkinglib
        return parkinglib.SHIM_serialise_for_parkingd(self.serialise())
