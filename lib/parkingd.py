...
class ParkingDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some parkingd methods
    ...
    @classmethod
    def from_parking_conf(self, parking_dot_conf):
        from parking_config import ParkingConfig
        config_text = ParkingConfig.slurp_config_file(parking_dot_conf)
        creds = ParkingConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = parkinglib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = parkinglib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = parkinglib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_parkingd_proto_version_with_sentinel_ping)
